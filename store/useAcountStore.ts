...
import { REWARD_CREATOR_ADDRESS } from "@/lib/constant";
  ...
  fetchAccountData: async (
    ...
    try {
      ...
      if (existsRewardSchedule) {
        claimableRewards = await getClaimableRewards(accountAddress);
      }

      const hasStake = await getUserHasStake(accountAddress);

      let accountStakeAmount = 0;
      if (hasStake) {
        const accountStakeData = await getUserStakeData(accountAddress);
      ...
      const onChainBalance = await getAccountTokenBalance(accountAddress);
